    ...
    if not os.path.exists(MASTER_FILE_NAME):
        ...
        with st.spinner("Processing volume aggregation matrices and compiling visual worksheets..."):
            try:
                ...
                # Robust Header Normalization: Strip out extra hidden spaces or casing variants automatically
                master_df.columns = [str(c).strip() for c in master_df.columns]
                
                # Dynamic column mapping to intercept minor naming shifts safely
                rename_map = {}
                for c in master_df.columns:
                    c_clean = c.lower().replace("-", " ").replace("_", " ").strip()
                    if "sub division" in c_clean: rename_map[c] = 'Sub_Division'
                    elif "sub office" in c_clean: rename_map[c] = 'Sub_Office'
                    elif "branch office" in c_clean or "office name" in c_clean: rename_map[c] = 'Office_Name'
                    elif "office id" in c_clean: rename_map[c] = 'Office_ID'
                    elif "type" in c_clean: rename_map[c] = 'Office_Type'
                
                master_df.rename(columns=rename_map, inplace=True)
                
                # Enforce rigid requirements mapping checks
                required_cols = ['Sub_Division', 'Sub_Office', 'Office_Name', 'Office_ID', 'Office_Type']
                missing_cols = [col for col in required_cols if col not in master_df.columns]
                
                if missing_cols:
                    st.error(f"🚨 Master File Alignment Error: Could not locate required columns. Checked file columns: {list(master_df.columns)}")
                    st.warning("Ensure your Master CSV contains columns named closely to: Sub Division, Sub Office, Branch Office, Office ID, office-type-code.")
                else:
